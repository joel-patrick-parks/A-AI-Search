...
vector<vector<vector<int>>> savedStates;
...
vector<string> gottemMoves;
vector<int> gottemSlots;
int winX;
int winY;
...
public:
	...
	//PieceMove getParentBoard();

	//void setParentBoard(PieceMove pieceMove);
...
vector<PieceMove> OPEN;
vector<PieceMove> CLOSED;
...
/*
vector<vector<int>> PieceMove::getParentBoard(){
	return parentBoard;
}

void PieceMove::setParentBoard(){
	parentBoard = pieceMove;
}
*/
	...
	for(int i = 0; i < pieceMoveBoard.size(); i++){
		for(int j = 0; j < pieceMoveBoard[i].size(); j++){
			if(pieceMoveBoard[i][j] == piece){
				...
				else if(masterBrickHeight == 2 && masterBrickWidth == 1){
...
void breadthFirstSearchSteps(){
	...
	for(int i = BSTStatesToSolution.size() - 2; i > 0; i--){
		if(i == 1){
			cout << "2l" << endl;
		}else{
...
int manhattanHeuristic(vector<vector<int>> board){
	int masterX = -2;
	int masterY;
	int disX;
	int disY;
	...
	disX = abs(winX - masterX);
	disY = abs(winY - masterY);
	disTotal = disX + disY;

	return disTotal;
}

int customHeuristic(vector<vector<int>> board){
	int masterX = -2;
	int masterY;
	int disX;
	int disY;
	...
	disX = (winX - masterX) * (winX - masterX);
	disY = (winY - masterY) * (winY - masterY);
	disTotal = disX + disY;

	return disTotal;
}

PieceMove removeLowestF(){
	int fValue = 500;
	int gValue = 0;
	int hValue = 0;
	int lowestFLocation = 0;
	...
	PieceMove lowestFValue(OPEN[lowestFLocation].getPieceBoard());
	OPEN.erase(OPEN.begin() + lowestFLocation);
	return lowestFValue;
}

void completeAStarAlgorithmManhattan(){
	while(!OPEN.empty()){
		PieceMove N = removeLowestF();
		gameBoard = N.getPieceBoard();
		gameBoardStates.push_back(normalizeState(gameBoard));
		parentToChild[parentToChild.size()-1].push_back(cloneState(gameBoard));
		parentToChildMoves[parentToChildMoves.size()-1].push_back("X");
		if(puzzleComplete(N.getPieceBoard())){
			breadthFirstSearchSteps();
			return;
		}
		CLOSED.push_back(N);
		//Total number of moves possible
		vector<string> possibleMoves = N.moveAllGeneration(); 
		for(int i = 0; i < possibleMoves.size(); i++){
			vector<string> currentMove;
			currentMove.resize(1);
			currentMove[0] = possibleMoves[i];
			bool isInClosedOrOpen = false;
			N.applyMove(currentMove);
			PieceMove M(N.getPieceBoard());
			//Checks to see if a state already exists
			if(!OPEN.empty()){
				for(int k = 0; k < OPEN.size(); k++){
					if(stateCompare(M.getPieceBoard(), OPEN[k].getPieceBoard())){
						isInClosedOrOpen = true;
					}
				}
			}
			if(!CLOSED.empty()){
				for(int j = 0; j < CLOSED.size(); j++){
					if(stateCompare(M.getPieceBoard(), CLOSED[j].getPieceBoard())){
						isInClosedOrOpen = true;
					}
				}
			}
			for(int q = 0; q < gameBoardStates.size(); q++){
				if(stateCompare(normalizeState(M.getPieceBoard()), gameBoardStates[q])){
					isInClosedOrOpen = true;
				}
			}
			//If not in OPEN or CLOSED add to OPEN
			if(!isInClosedOrOpen){
				M.parent = CLOSED.size()-1;
				gottemSlots.push_back(CLOSED.size()-1);
				M.g = N.g + 1;
				M.h = manhattanHeuristic(M.getPieceBoard());
				M.move = currentMove[0];
				gottemMoves.push_back(currentMove[0]);
				OPEN.push_back(M);
				gameBoardStates.push_back(M.getPieceBoard());
				nodes++;
				parentToChild[parentToChild.size()-1].push_back(cloneState(M.getPieceBoard()));
				parentToChildMoves[parentToChildMoves.size()-1].push_back(currentMove[0]);
			}
			N.setPieceBoard(gameBoard);
		}
		parentToChild.resize(parentToChild.size()+1);
		parentToChildMoves.resize(parentToChildMoves.size()+1);
	}
...
void completeAStarAlgorithmCustom(){
	while(!OPEN.empty()){
		PieceMove N = removeLowestF();
		gameBoard = N.getPieceBoard();
		gameBoardStates.push_back(normalizeState(gameBoard));
		...
		for(int i = 0; i < possibleMoves.size(); i++){
			...
			if(!isInClosedOrOpen){
				M.parent = CLOSED.size()-1;
				M.g = N.g + 1;
				M.h = customHeuristic(M.getPieceBoard());
				M.move = currentMove[0];
				OPEN.push_back(M);
				gameBoardStates.push_back(M.getPieceBoard());
				nodes++;
...
void aStarSearchManhattan(PieceMove pieceMove, int movesMade, bool firstRun){
	vector<string> possibleMoves;
	vector<string> currentMove;
	...
	int currentPosition;
	int bestBoard = 0;
	int currentBoardSlot = 0;
	bool smallestHeuristic = false;
	bool stateExists = false;
	...
	if(firstRun){
		...
		parentToChild.resize(parentToChild.size()+1);
		parentToChildMoves.resize(parentToChildMoves.size()+1);
		aStarSearchManhattan(pieceMove, numberOfMoves, firstRun);
	}else{
		...
		while(movesToBeMade < movesMade){
			...
			//heuristics.push_back(manhattanHeuristic());
		...
		//cout << "CHECKING HEURISTICS" << endl;
		for(int i = 0; i < heuristics.size(); i++){
			if(i == 0){
				bestBoard = heuristics[i];
			}else if(bestBoard > heuristics[i]){
				bestBoard = heuristics[i];
			}
		}

		if(bestBoard == 0){return;}

		//cout << "SMALLEST HEURISTIC: " << bestBoard << endl;

		while(movesToBeMade < movesMade){
			if(heuristics[currentBoardSlot] == bestBoard){
				gameBoard = cloneState(nonNormalBoardStates[currentPosition]);
				...
				savedStates.push_back(cloneState(nonNormalBoardStates[currentPosition]));
			}
			currentBoardSlot++;
			counter = 0;
			currentPosition++;
...
int main(){
	...
	*/
	...
	//Level 1
	string boardFile = "SBP-level2.txt";
	vector<vector<int>> gameBoardClone;
	parentToChild.resize(1);
	parentToChildMoves.resize(1);
	...
	pieceMove.g = 0;
	pieceMove.h = manhattanHeuristic(pieceMove.getPieceBoard());
	...
	OPEN.push_back(pieceMove);
	
	const clock_t m1_time = clock();
	completeAStarAlgorithmManhattan();
	cout << "Nodes: " << nodes << " Time: " << float(clock() - m1_time)/CLOCKS_PER_SEC << " Length: " << length << endl;
